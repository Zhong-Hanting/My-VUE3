<template>
    <div class="detailLocalStorageGet pagefn">
        ...
        <div class="fn-use">
            ...
            <code>}</code>
            <p class="note">// 调用方法：</p>
            <code><span class="yellow">this.proxy.localStorageSet(</span><span> keyname,object </span><span class="yellow">)；</span></code>
            <p class="note">// 参数说明：</p>
            <code><span>keyname</span><span class="red">:String</span> 需要存储localStorage中的key键名</code>
            <code><span>object</span><span class="red">:Object</span> 需要存储的对象</code>
        </div>
        <h3>函数代码</h3>
        <div class="fn-content">
            <code><span class="blue">localStorageSet</span>(<span>str, obj</span>) {</code>
            ...
            <code class="tab2">        localStorage.<span class="blue">setItem</span>(<span>str,stringify</span>)</code>
            <code class="tab2">        <span class="red">return</span> true;</code>
            <code class="tab1">    }</code>
            <code class="tab1">    else{</code>
            <code class="tab2">        console.error('$localStorageSet：参数错误')</code>
        ...
        <div class="demo">
            <section class="input">
                <p class="note">// 输入</p>
                <code><span class="yellow">this.proxy.localStorageSet</span>('<span>{{param}}</span>')；</code>
            </section>
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        data() {
            return {
                param: 'textLocalstorageKey,{"name":"legolas"}',
                result: ''
            }
        },
        setup() {
        ...
        methods: {
            ...
            run() {
                ...
                this.result = this.proxy.localStorageSet(param.k,param.v) || 'null';
            },
            reset() {
